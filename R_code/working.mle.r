...
for( i in 1:num.iter)
{
    ...
    if( i %% thin ==0)
    {
        ...
        this.clust = chain[[i/thin]][[4]]
        print(list(which(this.clust==0), which(this.clust==1), which(this.clust==2)))
    }
...
run.mle.with.cluster = function(data.set,num.states,num.ind,eps,lambda, cluster,pi,num.iter,thin){
